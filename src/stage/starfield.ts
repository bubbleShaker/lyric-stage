 ...
 *
 * **ただし M9-1（Issue #53）以降、そのまま差し替えても画にならない。** この実装は
 * 「暗い地に明るい星が浮かぶ」ことを前提に組んであり（星の色は `ink`、瞬きは
 * 明度を上げる向き）、退色した紙の上では**黒い星が散る**。戻すなら色と明滅の
 * 向きごと組み直すことになる — `GrainField` の中心の光をビネットへ反転したのと
 * 同じ作業。**「1 行で戻せる」のは配線の話で、画の話ではない。**
 *
