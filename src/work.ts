 ...
 * で本来の 7 行へ戻した。
 *
 * **終わりが「あと 10 秒」ちょうど（213.82）でないのは、拍と行の都合**
 * （[Issue #69](https://github.com/bubbleShaker/lyric-stage/issues/69)）。
 * 区間の終わりは拍の上に置く決まりがあり（`work.test.ts`）、213.82 はどの拍にも
 * 載らないうえ、最後の行「新たな世界へ」（`212.09`〜）を 1.7 秒で断ち切る。
 * 52 拍目まで伸ばすと +12.02 秒になり、最後の行が 5 拍ぶん出てフェードで閉じられる。
 *
