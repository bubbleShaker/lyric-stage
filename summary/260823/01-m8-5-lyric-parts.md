...
- **出番の前に語句が見えてしまう問題。** 語句は行の頭でまとめて組み立てるので、`at: 2.254` の
  ...
  それは**演出の書き方に頼った偶然**。隠す（`gsap.set(frame, { autoAlpha: 0 })`）のと
  見せる（`timeline.set(frame, { autoAlpha: 1 }, at)`）のを `buildLineTimeline` の 2 行に並べ、
  当て先を作る側に約束を配らない形にした
  - 隠す方をタイムラインに載せてはいけない。初回描画が次のフレームまで来ないぶん、
    **1 フレームだけ全語句が見えてしまう**
- **gsap は playhead が動いていないタイムラインを描き直さない。** 組み立てただけの状態で
  `time(0)` を当てても何も起きない（＝先頭で一時停止していると画面が空になる）。
  組み立ての最後に `time(0.0001).time(0)` と往復させて、時刻 0 の姿を確定させている
