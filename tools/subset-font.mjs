 ...
 */
/**
 * **シートに載らない文字がもう 1 か所ある**（M14-2）— 序の一文（`src/work.ts` の
 * `PRELUDE.text`）。ここは .mjs なので TS を読めず、集計に含められない。
 *
 * 今は本編の 1 行目と同じ一文なので歌詞から拾えているが、**別の一文に書き換えたら
 * 手でここに足すか、その字を含む歌詞があることを確かめること**。
 * 漏れは `src/font-subset.test.ts` の「序の一文の文字も持っている」が落とす。
 */
